...
这个项目使用现代 Android 技术栈，对 Smartisan OS 时代的锤子音乐播放器进行 1:1 视觉与交互复刻。目标很明确：在最新设备上，把原版黑胶唱盘、唱针阻尼、光影层次、页面切换和操作反馈重新做出来，同时补上现代系统环境下可持续维护的播放与数据基础设施。

本项目所有的逆向分析与代码实现，均由我与 Codex (GPT-5.4-xhigh) 通过持续对话合作完成。感谢 Codex 承担了全部的逆向与编码工作，我主要负责定义目标、挑选依赖、规划文件结构、把控工程品味与视觉审美、确认优先级、执行真机测试，并推动每一轮的迭代。

## 开源 Prompt

下面公开的是这个项目的主 Prompt。后续的大部分逆向整理、代码实现、UI 校正和细节收紧，都围绕这份 Prompt 持续展开。
...
时间悄无声息地带走了一些充满理想主义色彩的人和公司，也带走了一些曾经让我们眼前一亮的极致产品。

代码是可以被重写的，架构是可以被演进的，但那种对设计细节的死磕和对“美”的固执追求却越来越稀缺。锤子科技虽然已经离开舞台，但这套播放器依然代表着中文手机软件设计里极少见的一次高完成度表达。

复刻锤子音乐，不仅是为了怀旧，更是为了给自己和所有喜欢它的锤友们，重新打造一个在现代设备上真实可用、体验如初的本地音乐播放器。希望通过 Vibe Coding，能让那张黑胶唱片继续在屏幕上缓缓转动，保留住那份久违的精致与熟悉感。
